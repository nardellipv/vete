    ...
    <div class="content">
        ...
        <div class="panel panel-white">
            <div class="panel-heading">
                ...
                <small>Perido Desde {{ \Carbon\Carbon::parse($from)->format('d/m/Y') }} hasta {{ \Carbon\Carbon::parse($to)->format('d/m/Y') }}</small>
            </div>
            ...
            <table class="table table-lg invoice-archive">
                ...
                @foreach($searchDates as $searchDate)
                    <tr>
                        <td>#{{ $searchDate->invoice }}</td>
                        <td>#{{ $searchDate->invoice }}</td>
                        <td>
                            <h6 class="no-margin">
                                <a href="#">{{ $searchDate->customer->name }}</a>
                                <small class="display-block text-muted">{{ $searchDate->status }}</small>
                            </h6>
                        </td>
                        <td>{{ $searchDate->quantity }}</td>
                        <td>{{ \Carbon\Carbon::parse($searchDate->date)->format('d/m/Y') }}</td>
                        <td>${{ $searchDate->mount }}</td>
                        <td>${{ $searchDate->discount }}</td>
                        <td>${{ $searchDate->mount - $searchDate->discount }}</td>
